			    ...
			    console.log("Received message: " + e.payload);
                            ...
                            for (i in e.payload) {
                                if (i == "arrow") {
                                    ...
                                    req.open('GET', "https://peacock.place:8080/api/v1/entries/sgv.json?token=raj-6680c763a5eb003d&count=2", false);
                                    req.onload = function(e) {
                                        if (req.readyState == 4 && req.status == 200) {
                                            if(req.status == 200) {
...
*/
						now = new Date();
						now2 = now.getTime() - (20*60*1000);
						console.log(p[0].date + " " + now2);
						if (p[0].date < now2) {
			    ...
			    var options = JSON.parse(localStorage.getItem('clay-settings'));
                            if (options.updates == '1') {
                                Pebble.timelineSubscribe('updates',
                                                         ...
                                                         function (errorString) 
                                                         {
                                                             console.log('Error subscribing to updates: ' + errorString);
                                                         }
                                    );
                            }
